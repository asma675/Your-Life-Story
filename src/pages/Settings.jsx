...
export default function Settings() {
  ...
  const handleLogin = async () => {
    try {
      setSaving(true);
      const result = await base44.auth.login({ email, name });
      if (result?.user) {
        toast.success('Signed in');
        setUser(result.user);
        const redirect = sessionStorage.getItem('chronicle_post_login_redirect');
        if (redirect) {
          sessionStorage.removeItem('chronicle_post_login_redirect');
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <Card className="bg-slate-900/50 border-slate-800 glow-soft p-8">
        {!user && (
          <div className="mb-8 p-4 rounded-xl border border-slate-800 bg-slate-950/40">
            <h2 className="text-xl font-semibold text-white mb-1">Sign in</h2>
            <p className="text-slate-400 mb-4 text-sm">Enter an email to create a local account and sync your data via your backend.</p>
            ...
            <div className="mt-4">
              <Button onClick={handleLogin} disabled={saving || !email} className="bg-theme-solid hover-glow text-white">
